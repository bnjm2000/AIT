...
def test_maintenance_report_pdf_uses_only_committed_assets_and_generated_timestamp():
    ...
    assert "flushMaintenanceReportAssetInput" not in export_script
    assert "generatedAt: reportGeneratedAt()" in export_script
    assert "reportNumber" not in export_script
    assert "Generated on: ${safe(context.generatedAt)}" in pdf_script
    assert "Generated by: ${safe(context.generatedBy)}<br>" in pdf_script
    assert "GENERATED BY:<br>" not in pdf_script
    assert "No. :" not in pdf_script
...
def test_inventory_summary_pdf_uses_generated_timestamp_and_asset_count():
    ...
    pdf_script = script[pdf_start:pdf_end]
    export_start = pdf_end
    export_end = script.index("// Transfer grouping, actions, and grouped PDFs", export_start)
    export_script = script[export_start:export_end]

...
def build_worker_period_schedule_pdf(
    ...
    *,
    ...
    left_metadata = [("Worker", subject.get("name") or "-")]
    vendor_names = [
        str(name).strip()
    ...
    if show_vendor and vendor_names:
        left_metadata.append((
            "Vendor" if len(vendor_names) == 1 else "Vendors",
            ", ".join(vendor_names),
        ))
    left_metadata.append(("Date range", range_label or "-"))
    right_metadata = [
        ("Generated by", generated_by or "-"),
        ("Generated on", generated_at),
    ]
    metadata_rows = []
    for index in range(max(len(left_metadata), len(right_metadata))):
        left_entry = left_metadata[index] if index < len(left_metadata) else ("", "")
        right_entry = right_metadata[index] if index < len(right_metadata) else ("", "")
        metadata_rows.append([
            _paragraph(left_entry[0], label), _paragraph(left_entry[1], value),
            _paragraph(right_entry[0], label), _paragraph(right_entry[1], value),
        ])
    story = [

...
function findContainerInCacheByLookup(cache, lookup) {
  ...
  return Object.values(cache).find(container => {
    ...
    return (
      containerId === raw
      || (serial && serial.toLowerCase() === rawLower)
    );
  }) || null;
}
...
function currentUserRole() {
  return String(currentUser?.role || (currentUser?.isSuperAdmin || currentUser?.isAdmin ? 'admin' : 'user')).toLowerCase();
}
...
let maintenanceIdentifierLookupInProgress = false;
...
async function ensureMaintenanceAssetsForSelection(assetIds) {
  const identifiers = Array.isArray(assetIds) ? assetIds : [];
  const hasMissingAssets = identifiers.some(identifier => (
    !getAssetByApiIdentifier(identifier)
    && !findAssetByIdentifier(String(identifier || ''), assets)
  ));
  if (!hasMissingAssets) return;

  const response = await apiCall(`/api/assets?_=${Date.now()}`);
  if (!response?.success || !Array.isArray(response.data)) {
    throw new Error('Unable to load the assets in this container');
  }
...
async function addContainerToMaintenanceSelection(container) {
  ...
  try {
    await ensureMaintenanceAssetsForSelection(assetIds);
  } catch (error) {
    console.error(`Unable to load assets for container ${containerId}:`, error);
    showNotification(
      'error',
      error.message || `Unable to load assets from container ${containerId}`
    );
    return;
  }

  if (typeof window.addAssetsToMaintenanceSelection !== 'function') {
    throw new Error('Maintenance asset selection is not ready');
  }
  const selectionResult = window.addAssetsToMaintenanceSelection(assetIds);
  if (searchEl) searchEl.value = '';
  if (typeof window.searchMaintenanceAssets === 'function') {
    window.searchMaintenanceAssets();
  }
...
async function addIdentifierToMaintenanceSelection(identifier) {
  ...
  if (asset) {
    if (typeof window.selectAssetForMaintenance !== 'function') {
      throw new Error('Maintenance asset selection is not ready');
    }
    window.selectAssetForMaintenance(getAssetIdentifierForApi(asset));
    const searchEl = document.getElementById('maintenanceAssetSearch');
    if (searchEl) searchEl.value = '';
    return;
  }

  const endpoint = `/api/containers/${encodeURIComponent(normalized)}?_=${Date.now()}`;
  ...
  if (responseText) {
    try {
      containerResponse = JSON.parse(responseText);
    } catch (error) {
      throw new Error('The container lookup returned an invalid response');
    }
  }
  ...
  if (!response.ok) {
    throw new Error(containerResponse.error || 'Unable to check this container');
  }

  const container = containerResponse.data || null;
  if (container) {
    await addContainerToMaintenanceSelection(container);
    return;
  }
...
async function handleMaintenanceIdentifierEntry(event) {
  if (event.key !== 'Enter') return;
  event.preventDefault();
  event.stopPropagation();
  if (maintenanceIdentifierLookupInProgress) return;

  const input = event.currentTarget;
  const identifier = normalizeScannedIdentifier(input?.value);
  if (!identifier) return;

  maintenanceIdentifierLookupInProgress = true;
  input.readOnly = true;
  input.setAttribute('aria-busy', 'true');
  const results = document.getElementById('availableMaintenanceAssets');
  if (results) {
    results.innerHTML = '<div style="padding: 20px; text-align: center; color: #66736c;">Checking asset or container...</div>';
  }

  try {
    await addIdentifierToMaintenanceSelection(identifier);
  } catch (error) {
    console.error(`Maintenance identifier lookup failed for ${identifier}:`, error);
    if (results) {
      const message = error?.message || 'Unable to check this identifier. Please try again.';
      results.innerHTML = `<div style="padding: 20px; text-align: center; color: #b42318;">${escapeHtml(message)}</div>`;
    }
  } finally {
    maintenanceIdentifierLookupInProgress = false;
...
async function apiCall(endpoint, method = "GET", data = null) {
  try {
    ...
    if (
      ...
      && error.payload?.code !== 'document_version_conflict'
      && error.payload?.code !== 'asset_import_plan_stale'
    ) {
...
const EVENT_FILES_TOTAL_LIMIT_BYTES = 20 * 1024 * 1024;

function closeEventFilePreview() {
  const body = document.getElementById('eventFilePreviewBody');
  if (body) body.replaceChildren();
  closeModal('eventFilePreviewModal');
}

function previewEventFile(eventId, filename, previewUrl, previewKind, downloadUrl) {
  const title = document.getElementById('eventFilePreviewTitle');
  const body = document.getElementById('eventFilePreviewBody');
  const download = document.getElementById('eventFilePreviewDownload');
  if (!title || !body || !download) return;

  const safeName = String(filename || 'Event file');
  const fileUrl = String(
    downloadUrl ||
    `/api/events/${Number(eventId)}/files/${encodeURIComponent(safeName)}`
  );
  const inlineUrl = String(previewUrl || '');
  const kind = String(previewKind || '');
  ...
  if (kind === 'image' && inlineUrl) {
    ...
    preview.alt = safeName;
  } else if (kind === 'video' && inlineUrl) {
    preview = document.createElement('video');
    preview.controls = true;
  } else if (kind === 'audio' && inlineUrl) {
    preview = document.createElement('audio');
    preview.controls = true;
  } else if ((kind === 'pdf' || kind === 'text') && inlineUrl) {
    preview = document.createElement('iframe');
    preview.title = `Preview of ${safeName}`;
  ...
  if (preview) {
    ...
    unavailable.className = 'event-file-preview-unavailable';
    unavailable.textContent = 'A browser preview is not available for this file type. Download the file to open it.';
    body.appendChild(unavailable);
  }

  openModal('eventFilePreviewModal');
}
...
function renderEventFilesList(eventId, files) {
  ...
  return list.map(file => {
    ...
    const previewKind = String(file.previewKind || '');
    ...
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 10px 12px; border: 1px solid #e9ecef; border-radius: 8px; background: #fff; margin-bottom: 8px;">
        <div style="min-width: 0; flex: 1;">
          <button type="button" onclick="previewEventFile(${eventId}, '${escapeJs(name)}', '${escapeJs(previewUrl)}', '${escapeJs(previewKind)}', '${escapeJs(downloadUrl)}')" style="display:block;width:100%;padding:0;border:0;background:transparent;color:#344054;font:inherit;font-weight:600;text-align:left;overflow-wrap:anywhere;cursor:pointer;">${escapeHtml(name)}</button>
...
async function uploadEventFileSelection(eventId, fileList) {
  ...
  if (selectedSize > EVENT_FILES_TOTAL_LIMIT_BYTES) {
    showNotification("error", "Selected files exceed the 20 MB total event limit");
    return false;
  }
...
async function loadAssetUsageDays(assetId) {
  const content = document.getElementById('assetDetailsContent');
  const value = document.getElementById('assetDetailsUsageDays');
  if (!content || !value) return;

  try {
    const response = await apiCall(
      `/api/assets/${encodeURIComponent(assetId)}/usage-summary`
    );
    if (content.dataset.assetId !== String(assetId || '')) return;
    const days = Math.max(0, Number(response.data?.usageDays || 0));
    value.textContent = `${days} day${days === 1 ? '' : 's'}`;
  } catch (error) {
    if (content.dataset.assetId === String(assetId || '')) {
      value.textContent = '-';
    }
  }
}
...
function inventoryOpenContainer(clickEvent, encodedContainerId) {
  clickEvent?.preventDefault();
  clickEvent?.stopPropagation();
  let containerId = String(encodedContainerId || '');
  try { containerId = decodeURIComponent(containerId); } catch (error) {}
  if (containerId) viewContainer(containerId);
}

function inventoryAssetContainersHtml(asset) {
  const memberships = Array.isArray(asset?.containers) ? asset.containers : [];
  if (!memberships.length) return '<span class="inventory-no-container">—</span>';
  return `<div class="inventory-container-memberships">${memberships.map(membership => {
    const containerId = String(membership?.id || '').trim();
    if (!containerId) return '';
    const encodedContainerId = encodeURIComponent(containerId).replace(/'/g, '%27');
    const quantity = Math.max(1, Number.parseInt(membership?.quantity, 10) || 1);
    const quantityLabel = asset?.isBulk ? ` · ${quantity}` : '';
    const accessibleQuantity = asset?.isBulk ? `, ${quantity} units` : '';
    return `<button type="button" class="inventory-container-badge" title="View container ${escapeHtmlAttr(containerId)}" aria-label="Container ${escapeHtmlAttr(containerId)}${accessibleQuantity}" onclick="inventoryOpenContainer(event,'${escapeHtmlAttr(encodedContainerId)}')">${inventoryIcon('box')}<span>${escapeHtml(containerId)}${escapeHtml(quantityLabel)}</span></button>`;
  }).join('')}</div>`;
}
...
function inventoryIndividualRowHtml(asset, isAdmin) {
  ...
  return `
    <div class="inventory-individual-row ${isSelected ? 'is-selected' : ''}">
      ...
      <div>${inventoryAssetContainersHtml(asset)}</div>
            ...
            ${expanded ? `
              <div class="inventory-model-detail">
                ...
                <div class="inventory-individual-list">
                  <div class="inventory-individual-head"><span></span><span>Asset ID</span><span>Version</span><span>Container</span><span>Availability</span><span>Location</span><span>Last maintenance</span><span></span></div>
...
async function saveBulkAssetEditModal() {
  ...
  const groupFieldsSelected = ['department', 'brand', 'model', 'description']
    .some(field => Object.prototype.hasOwnProperty.call(samplePayload, field));
  if (groupFieldsSelected) {
    const selectedAssetIds = new Set(selectedAssets.map(inventoryAssetIdentifier));
    const changedDestinationGroupKeys = new Set(selectedAssets.map(asset => {
      const destination = {
        department: Object.prototype.hasOwnProperty.call(samplePayload, 'department')
          ? samplePayload.department
          : asset.department,
        brand: Object.prototype.hasOwnProperty.call(samplePayload, 'brand')
          ? samplePayload.brand
          : asset.brand,
        model: Object.prototype.hasOwnProperty.call(samplePayload, 'model')
          ? samplePayload.model
          : asset.model,
        description: Object.prototype.hasOwnProperty.call(samplePayload, 'description')
          ? samplePayload.description
          : asset.description,
      };
      return inventoryAssetGroupKey(destination) !== inventoryAssetGroupKey(asset)
        ? inventoryAssetGroupKey(destination)
        : '';
    }).filter(Boolean));
    const existingDestinationAssets = assets.filter(asset => (
      !selectedAssetIds.has(inventoryAssetIdentifier(asset))
      && changedDestinationGroupKeys.has(inventoryAssetGroupKey(asset))
    ));

    if (existingDestinationAssets.length) {
      const confirmed = await showAppConfirm({
        title: 'Asset groups will be merged',
        message:
          `${selectedAssets.length} selected asset${selectedAssets.length === 1 ? '' : 's'} will join ` +
          `${existingDestinationAssets.length} unselected asset${existingDestinationAssets.length === 1 ? '' : 's'} already in the destination group.\n\n` +
          'After saving, they will appear together as one inventory model group.',
        checkboxLabel: 'I understand that these assets will be merged into the existing model group.',
        requireCheckbox: true,
        confirmText: 'Merge and Save',
  ...
  if (samplePayload.__selectedFieldCount) {
    for (const asset of selectedAssets) {
      ...
      if (groupFieldsSelected) {
        // Sequential updates make the first selected asset the destination
        // group for the remaining selected assets. The multi-selection itself
        // authorizes that intended merge; external destinations were confirmed
        // once above before any asset was changed.
        payload.confirmModelGroupMerge = true;
      }
...
function handleContainerAction(button) {
  const action = String(button?.dataset?.containerAction || '');
  const containerId = String(button?.dataset?.containerId || '');
  if (!action || !containerId) return;

  const handlers = {
    maintain: openContainerMaintenanceModal,
    view: viewContainer,
    edit: editContainer,
    save: saveContainerEdit,
    delete: deleteContainer,
    'remove-maintenance-filter': removeMaintenanceReportContainerFilter,
  };
  return handlers[action]?.(containerId);
}
    ...
    return `
      <div class="container-card-modern">
        ...
        <div class="container-card-actions">
          <button class="inventory-icon-button" title="Log maintenance" aria-label="Log maintenance" data-container-action="maintain" data-container-id="${escapeHtmlAttr(container.id)}" onclick="handleContainerAction(this)">${inventoryIcon('wrench')}</button>
          <button class="inventory-icon-button" title="View container" aria-label="View container" data-container-action="view" data-container-id="${escapeHtmlAttr(container.id)}" onclick="handleContainerAction(this)">${inventoryIcon('eye')}</button>
          <button class="inventory-icon-button" title="Edit container" aria-label="Edit container" data-container-action="edit" data-container-id="${escapeHtmlAttr(container.id)}" onclick="handleContainerAction(this)">${inventoryIcon('edit')}</button>
        </div>
      </div>`;
  }).join('');
}
...
function makeContainerEditorHtml(mode, container = null) {
  ...
  return `
    ...
    <div style="display:flex;gap:10px;justify-content:space-between;margin-top:18px;">
      ${
        isEdit
          ? `<button class="btn btn-danger" data-container-action="delete" data-container-id="${escapeHtmlAttr(containerId)}" onclick="handleContainerAction(this)">Delete Container</button>`
          : `<div></div>`
      }
      <div style="display:flex;gap:10px;">
        ...
        ${
          isEdit
            ? `<button class="btn btn-success" data-container-action="save" data-container-id="${escapeHtmlAttr(containerId)}" onclick="handleContainerAction(this)">Save Changes</button>`
      ...
      <div class="container-hero" style="margin-bottom:16px;">
        ...
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="btn btn-success" data-container-action="maintain" data-container-id="${escapeHtmlAttr(c.id)}" onclick="handleContainerAction(this)">Log Maintenance</button>
            <button class="btn btn-primary" data-container-action="edit" data-container-id="${escapeHtmlAttr(c.id)}" onclick="handleContainerAction(this)">Edit Container</button>
  ...
  const containerChips = selectedContainers.map(containerId => {
    ...
    return `
      <div class="maintenance-report-chip container-chip" title="Container">
        ...
        <button type="button" data-container-action="remove-maintenance-filter" data-container-id="${escapeHtmlAttr(containerId)}" onclick="handleContainerAction(this)" aria-label="Remove ${escapeHtmlAttr(containerId)}">&times;</button>
      </div>
    `;
  });
...
function removeMaintenanceReportContainerFilter(containerId) {
...
function maintenanceReportFilterSummary(rows) {
  ...
  if (start || end) {
    const startLabel = start ? maintenanceReportDateForDisplay(start) : 'All';
    const endLabel = end ? maintenanceReportDateForDisplay(end) : 'Now';
    parts.push(`Date range: ${startLabel} to ${endLabel}`);
  }
...
function eventOverviewIcon(kind) {
  const paths = {
    ...
    rooms: '<rect x="3" y="4" width="18" height="16" rx="2"></rect><path d="M12 4v16M3 11h9M12 14h9"></path>',
...
function eventOverviewSubprojectRows(event) {
  const rooms = Array.isArray(event?.subprojects)
    ? event.subprojects.filter(Boolean).map(room => ({
        ...room,
  ...
  const modelItems = Object.values(event?.modelGroups || {})
    .filter(group => Number(group?.requiredQuantity || 0) > 0)
    .map(group => ({
      brand: group.brand || '',
      model: group.model || '',
      description: group.description || '',
      quantity: Number(group.requiredQuantity || 0)
    }));
  const customItems = groupEventCustomAssets(getCustomAssetsFromEvent(event)).map(row => ({
    brand: '',
    model: '',
    description: customAssetDisplayName(row.parsedCustom, false),
    quantity: Number(row.parsedCustom?.quantity || 0)
  }));

  return [{
    id: 'main',
    name: 'Main Room',
    items: [...modelItems, ...customItems]
  }];
}

function eventOverviewSubprojects(event) {
  const rooms = eventOverviewSubprojectRows(event);
  return `<div class="event-overview-subprojects">${rooms.map(room => {
    const items = (room.items || []).filter(item => item && Number(item.quantity || 0) > 0);
    ...
    return `<article class="event-overview-subproject">
      <div><strong>${escapeHtml(room.name || 'Unnamed room')}</strong><span>${items.length} line${items.length === 1 ? '' : 's'} · ${quantity} required</span></div>
      ${labels.length ? `<div class="event-overview-subproject-items">${labels.slice(0, 6).map(label => `<span>${escapeHtml(label)}</span>`).join('')}${labels.length > 6 ? `<span>+${labels.length - 6} more</span>` : ''}</div>` : '<small>No asset requirements</small>'}
    </article>`;
  }).join('')}</div>`;
}
...
function eventOverviewFiles(event) {
  ...
  return files.map(file => {
    ...
    const deleteButton = isAdminUser()
      ...
      : '';
    return `<div class="event-overview-file"><div><button type="button" class="event-overview-file-preview" onclick="previewEventFile(${Number(event.id)}, '${escapeJs(name)}', '${escapeJs(previewUrl)}', '${escapeJs(previewKind)}', '${escapeJs(url)}')"><strong>${escapeHtml(name)}</strong><span>${formatEventFileSize(file.size || 0)}${file.modifiedAt ? ` · ${escapeHtml(formatEventFileModified(file.modifiedAt))}` : ''}</span></button></div><div class="event-overview-file-actions"><a href="${escapeHtmlAttr(url)}">Download</a>${deleteButton}</div></div>`;
  }).join('');
}
...
function eventOverviewFilesPanel(event) {
  ...
  return `<div class="event-overview-files-panel">
    ...
    <label id="eventOverviewFileDropzone-${eventId}" class="event-overview-file-dropzone"
      ...
      <span><strong>Drop files here</strong><small id="eventOverviewUploadState-${eventId}">or choose files · 20 MB total</small></span>
    </label>
...
function eventOverviewInternalUsers(event) {
  ...
  return `<div class="event-overview-internal-users">${rows.map(row => `<span class="event-overview-pill">${escapeHtml(eventAssigneeDisplayName(row) || row.username || 'User')} · ${escapeHtml(row.roleLabel || userRoleLabel(row.role))}</span>`).join('')}</div>`;
}
...
function maintenanceUploadLimitHtml() {
  return `<small class="maintenance-upload-limit">${escapeHtml(maintenanceUploadLimitText())}</small>`;
}
...
function populateEditQuantityModal(eventId, brand, model, department, currentQuantity, description) {
  ...
  document.getElementById("editQuantityInput").removeAttribute("max");
...
function validateQuantityInput(inputId) {
  const input = document.getElementById(inputId);
  if (!input) return;
...
function handleQuantityBlur(inputId) {
  const input = document.getElementById(inputId);
  if (!input) return;
...
function modelAvailabilityReasonTooltip(available, physical, reasons = {}) {
  ...
  if (overlappingEvents.length > 0) {
    overlappingEvents.forEach(event => {
      ...
      const eventLabel = eventId ? `#${eventId}: ${eventName}` : eventName;
      ...
      rows.push(
        `${quantity} ${quantity === 1 ? 'asset is' : 'assets are'} used by ${eventLabel}` +
        `${dateRange ? ` (${dateRange})` : ''}`
      );
    });
  ...
  filteredModels.slice(0, 20).forEach(model => {
    ...
    html += `
      <div style="padding: 12px; border-bottom: 1px solid #f1f1f1; display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="display: flex; align-items: center; gap: 10px;">
          <input type="number" id="${qtyInputId}" min="1" value="1"
              style="width: 60px; padding: 4px; border: 1px solid #ddd; border-radius: 4px; font-size: 12px;"
              oninput="validateQuantityInput('${qtyInputId}'); updateModelAvailabilityLabel('${qtyInputId}')"
              onblur="handleQuantityBlur('${qtyInputId}'); updateModelAvailabilityLabel('${qtyInputId}')"
...
function positionalAssetSerialList(value) {
...
function addAssetSerialList(fieldId = 'assetSerials') {
  return positionalAssetSerialList(addAssetField(fieldId)?.value || '');
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (maintenanceAssetSearch) {
    ...
    maintenanceAssetSearch.addEventListener(
      "keydown",
      handleMaintenanceIdentifierEntry
    );
  }
...
window.addAssetsToMaintenanceSelection = addAssetsToMaintenanceSelection;
window.searchMaintenanceAssets = searchMaintenanceAssets;
window.selectAssetForMaintenance = selectAssetForMaintenance;
...
function exportLogs() {
  ...
  const includeCompany = isPlatformAdminUser();
...
async function refreshEventOverviewViews() {
  ...
  if (document.getElementById('return-section')?.classList.contains('active')) {
    refreshes.push(loadReturnWorkspace());
  }

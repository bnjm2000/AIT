...
def make_maintenance_log(date, user, description, changes=None):
...
def status_change_labels(log_or_changes):
    ...
    for change in changes:
        ...
        if kind == 'location':
            ...
            labels.append("Marked OOC" if change.get('action') == 'marked' else "Cleared OOC")
        elif kind == 'missing':
            labels.append("Marked Missing" if change.get('action') == 'marked' else "Cleared Missing")
...
def apply_maintenance_log_changes(asset, log):
    for change in normalize_maintenance_log(log).get('changes', []):
        ...
        if kind == 'location':
            ...
            asset.is_ooc = change.get('action') == 'marked'
        elif kind == 'missing':
            asset.is_missing = change.get('action') == 'marked'

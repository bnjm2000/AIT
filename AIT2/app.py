...
def _workforce_folder(manager=None):
    manager = manager or _current_data_manager_object()
    return os.path.abspath(getattr(manager, 'data_folder', '') or '')
...
def _worker_token_data(token):
    try:
        payload = _worker_token_serializer().loads(
            str(token or ''),
            max_age=WORKER_TOKEN_MAX_AGE,
        )
    except SignatureExpired as exc:
        raise ValueError('Your worker session has expired. Enter your phone number again.') from exc
    except BadSignature as exc:
        raise ValueError('Invalid worker session. Enter your phone number again.') from exc
    if not isinstance(payload, dict):
        raise ValueError('Invalid worker session')
    company_code = _normalise_company_code(payload.get('companyCode'))
...
def _bind_public_company_manager(company_code):
    ...
    current = _current_data_manager_object()
    if manager is not current and hasattr(manager, 'acquire_write_request'):
        manager.acquire_write_request()
...
def _event_date_for_worker(value):
    try:
...
def _worker_company_payload(company_code, manager, freelancer, workforce):
    ...
    department_rows = _department_codes_for_manager(manager)
    departments_by_code = {
        row['code']: row for row in department_rows
    }
    ...
    for raw_event_id, assignments in assignments_by_event.items():
        ...
        for assignment in assignments:
            ...
            department_details = (
                ...
                or _department_record(department)
            )
            ...
            assignment_payload.append({
                ...
                'departmentTextColor': department_details['textColor'],
...
def _find_worker_context(token, event_id=None):
    ...
    if event_id is not None:
        ...
        if not assigned or int(event_id) not in manager.events:
            raise ValueError('You are not assigned to this event')
    return company_code, manager, workforce, freelancer
...
def _department_codes_for_manager(manager):
    ...
    if not departments:
        departments = {
            code: dict(row) for code, row in DEFAULT_DEPARTMENTS.items()
        }
    payload = []
    for code, record in departments.items():
        if isinstance(record, dict):
            name = record.get('name') or record.get('Name') or code
            colour = record.get('color') or record.get('Color')
            text_colour = record.get('textColor') or record.get('TextColor')
        else:
            name = getattr(record, 'name', '') or code
            colour = getattr(record, 'color', '')
            text_colour = getattr(record, 'text_color', '')
        payload.append(_department_record(
            code, name, colour, text_colour
        ))
...
def _event_workforce_departments(event_id, event, manager, workforce):
    ...
    for row in manual_rows:
        ...
        manual_by_code[code] = {
            ...
            'color': (configured.get(code) or {}).get('color'),
            'textColor': (configured.get(code) or {}).get('textColor'),
            'source': 'manual',
        }
        active_codes.add(code)
    ...
    for code in sorted(active_codes):
        ...
        active_rows.append({
            ...
            'color': _normalise_hex_colour(base.get('color')),
            'textColor': _normalise_hex_colour(
                base.get('textColor')
                or _best_text_colour(base.get('color')),
                _best_text_colour(base.get('color')),
            ),
            'source': (
                'manual'
...
@app.route('/api/maintenance/users', methods=['GET'])
@require_auth
def get_maintenance_user_options():
    """List active company users available for maintenance attribution."""
    ...
    for user in data_manager.users.values():
        ...
        if assigned_company != current_company:
            continue
        display_name = str(getattr(user, 'name', '') or '').strip() or username
        rows.append({
            'username': username,
            'name': display_name,
        })

    rows.sort(key=lambda row: (row['name'].lower(), row['username'].lower()))
...
def get_event(event_id):
    ...
    try:
        ...
        # Also process physical model-fulfilled assets that are not represented
        # directly in prepared_items. Returned regular assets are removed from
        # actually_prepared, so include returned_items as well; this keeps the
        # Return workspace history complete and makes accidental returns
        # reversible without reconstructing asset metadata in the browser.
        physical_history_refs = list(dict.fromkeys(
            list(event.actually_prepared or []) +
            list(event.returned_items or [])
        ))
        for asset_id in physical_history_refs:
...
def unreturn_event_asset(event_id):
    ...
    try:
        ...
        _ensure_event_custom_lists(event)
        if asset_id not in event.returned_items:
            return jsonify({'error': 'Asset is no longer marked as returned'}), 409

        custom = _parse_custom_marker(asset_id)
        bulk = _parse_bulk_marker(asset_id)
        ...
        if bulk:
            ...
            if busy_event:
                return jsonify({
                    'error': (
                        f'Asset is now assigned to event {busy_event.event_id}: '
                        f'{busy_event.name}'
                    )
                }), 409
        ...
        return jsonify({
            'success': True,
            'message': f'Asset {asset_id} restored to event',
            'data': {
                'assetId': asset_id,
                'eventId': event_id,
                'state': event.state,
        ...
        logger.error(
            f"Error undoing return of asset for event {event_id}: {e}",
            exc_info=True,
        )
        return jsonify({'error': 'Failed to undo asset return'}), 500
...
def close_return_event(event_id):
    ...
    try:
        ...
        counts = _event_returnable_counts(event)
        if counts['returnable'] > 0:
            return jsonify({
                'error': f"{counts['returnable']} asset(s) still need to be returned"
            }), 409
        if counts['total'] <= 0:
        ...
        logger.error(f"Error closing returned event {event_id}: {e}", exc_info=True)
...
def _maintenance_log_user_from_request(data):
    ...
    current_username = str(session.get('user') or '').strip()
    explicit_username = (data or {}).get('maintenanceUser') or (data or {}).get('logUser')
    if not explicit_username:
        return current_username, ''
    requested_username = str(explicit_username).strip()
    user = data_manager.users.get(requested_username)
    if not user or not getattr(user, 'is_active', True):
        return None, 'Select an active user from your company'

    current_company = _normalise_company_code(
        _current_company_code(),
        DEFAULT_COMPANY_CODE,
    )
    user_company = _normalise_company_code(
        _user_assigned_company_code(requested_username),
        DEFAULT_COMPANY_CODE,
    )
    if user_company != current_company:
        return None, 'Select an active user from your company'
    if _is_owner_username(requested_username) and not _current_user_is_owner():
        return None, 'Select an active user from your company'
    return requested_username, ''


def _maintenance_log_source_for_request(data, source=None):
    """Attach request de-duplication and recorder attribution to log metadata."""
    metadata = dict(source or {})
    request_id = _maintenance_client_request_id(data)
    if request_id:
        metadata['clientRequestId'] = request_id

    attributed_user, error = _maintenance_log_user_from_request(data)
    if error:
        return None, None, error
    recorder = str(session.get('user') or '').strip()
    if attributed_user != recorder:
        metadata['recordedBy'] = recorder
    return attributed_user, metadata, ''
...
def _apply_standard_maintenance_request(asset_id, asset, data, uploaded_files=None):
    ...
    maintenance_user, source, attribution_error = _maintenance_log_source_for_request(data)
    if attribution_error:
        return {'error': attribution_error}, 400, False
...
def update_maintenance_log_enhanced(asset_id, log_index):
    ...
    try:
        ...
        if not _current_user_is_admin():
            # The original attribution is restored below after the log is loaded.
        ...
        if not _current_user_is_admin():
            # A recorder editing an on-behalf-of entry must not take authorship.
            new_user = str(original_log.get('user') or '').strip()
...
def maintain_container(container_id):
    ...
    try:
        ...
        maintenance_user, attribution_source, attribution_error = (
            _maintenance_log_source_for_request(data)
        )
        if attribution_error:
            return jsonify({'error': attribution_error}), 400
